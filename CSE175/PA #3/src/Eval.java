...
// Header Comment:
// In this Programming Assignment, we only implement function value_roll_hand and 
// function heuristic. For value_roll_hand, we find out all possible outcomes of 
// dice roll and calculate utility values for those outcome. We can call value_rolled_hand
// to do the actual calculation. Heuristic function is used to calculate the quality of 
// current state for computer and it return a real number to grade the quality. If the
// number returned is positive, the current state is good for computer, whereas, user is in
// good place. 
...
public class Eval {
	...
	// Following three for-loops are used to generate all possible
	// outcomes for three dice. DieFace will become invalid, but we don't want
	// encounter such situation. Thus, when die is invalid, we use
	// continue to skip it.
	for (DieFace die1 : DieFace.values()) { // choose a symbol for first die
		// don't let the first die become invalid
		if (die1 == DieFace.invalid) {
			continue; 
		} else {
			for (DieFace die2 : DieFace.values()) { // choose a symbol for the second die
				// don't let the second die become invalid
				if (die2 == DieFace.invalid) {
					continue;
				} else {
					for (DieFace die3 : DieFace.values()) { // choose a symbol for the third die
						// don't let the third die become invalid
						if (die3 == DieFace.invalid) {
							continue;
						} else {
							// calculate the probability of value of current symbol value for three dice
							double prob = s.rollProb(die1, die2, die3);
							// roll the three dice
							s.roll(die1, die2, die3);
							// calculate the value for rolled dice
							val += prob * value_rolled_hand(s, depth);
						}
					}
				}
			}
    ...
    static public double heuristic (State s) {
		...
		// calculate the difference between # of brains that computer has and user_brains_eaten
		// if diff_comp_user is negative, that means user has more brains
		// if diff_comp_user is positive, that means computer has more brains
		double diff_comp_user = s.comp_brains_eaten + s.brains_collected - s.user_brains_eaten;

		// The one who first get 13 brains wins the game. Divide diff_comp_user to 13,
		// we get how far is the game. Times the win_payoff, we get the heuristic value
		// finally we get the quality of current state
