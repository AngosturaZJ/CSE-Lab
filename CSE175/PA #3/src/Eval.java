...
public class Eval {
    ...
    static double value_roll_hand (State s, int depth) {
        ...
        // calculate the utility value for every single situation
        for (DieFace x : DieFace.values())  {
            // calculate the utility value when each die has a particular symbol
            if (x ==  DieFace.invalid) {
                continue; // we want know the particular symbol
            } else {
                    ...
                    if (y  ==  DieFace.invalid) {
                        ...
                        for (DieFace z : DieFace.values()) {
                            if (z == DieFace.invalid) {
                                ...
                                // calculate the probability for current situation (each die has a symbol)
                                double prob = s.rollProb(x, y, z);
                                s.roll(x, y, z);
